...
@cache
def _compile_chunk_h_fwd(input_dtype, state_dtype, K: int, V: int, *, use_initial_state: bool, store_final_state: bool):
    class ChunkHForward:
        @cute.jit
        def __call__(
            self,
        ...
        def kernel(
            ...
            if state_idx < state_count:
                ...
                mH[state_idx] = state_dtype(state)
    ...
    h_fake = cute.runtime.make_fake_tensor(state_dtype, (cute.sym_int(),), stride=(1,), assumed_align=16)
    initial_state_fake = (
...
def chunk_h_fwd_cute(
    ...
    """Run the dense ungated single-chunk CuTe state forward kernel."""
    B, T, H, K = k.shape
    V = v.shape[-1]
    state_dtype = torch.float32 if states_in_fp32 else k.dtype
    h = k.new_empty(B, 1, H, K, V, dtype=state_dtype)
    final_state = k.new_empty(B, H, K, V, dtype=torch.float32) if output_final_state else None
    compiled = _compile_chunk_h_fwd(
        _torch_to_cute_dtype(k.dtype),
        _torch_to_cute_dtype(state_dtype),
        K,
        V,
