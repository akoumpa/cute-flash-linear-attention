...
@cache
def _compile_chunk_h_split_fwd(
    input_dtype,
    state_dtype,
    K: int,
    V: int,
    *,
    ...
    class ChunkHSplitForward:
        ...
        def __call__(
            ...
            state_count = B * H * K * V
            self.kernel(mK, mV, mHR, mH0, mHT, T, H, state_count).launch(
        ...
        def kernel(
            ...
            if state_idx < state_count:
                ...
                # hr stores the state entering the sole split.
                mHR[state_idx] = state_dtype(state)
    ...
    hr_fake = cute.runtime.make_fake_tensor(state_dtype, (cute.sym_int(),), stride=(1,), assumed_align=16)
    h0_fake = (
...
def chunk_h_split_fwd_cute(
    ...
    """Run the dense single-split CuTe state-passing forward kernel."""
    B, T, H, K = k.shape
    V = v.shape[-1]
    hr = k.new_empty(B, H, K, V, dtype=torch.float32 if states_in_fp32 else k.dtype)
    ht = k.new_empty(B, H, K, V, dtype=torch.float32) if output_final_state else None
    compiled = _compile_chunk_h_split_fwd(
        _torch_to_cute_dtype(k.dtype),
        _torch_to_cute_dtype(hr.dtype),
        K,
        V,
...
__all__ = ["chunk_h_split_fwd_cute"]
