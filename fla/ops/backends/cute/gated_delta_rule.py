...
@cache
def _compile_gated_delta_rule_fwd(
    input_dtype,
    gate_dtype,
    beta_dtype,
    *,
    ...
    class GatedDeltaRuleForward:
        ...
        def kernel(
            ...
            for pos in cutlass.range(0, T, 1, unroll=1):
                ...
                decay = exp(Float32(mG[value_token]))
                erase = Float32(0.0)

                for key_idx in cutlass.range_constexpr(K):
                    state[key_idx] *= decay
                    erase += state[key_idx] * Float32(mK[key_token * K + key_idx])

                value_new = (Float32(mV[value_token * V + value_idx]) - erase) * Float32(mBeta[value_token])
                output = Float32(0.0)
                for key_idx in cutlass.range_constexpr(K):
                    key = Float32(mK[key_token * K + key_idx])
                    state[key_idx] += key * value_new
                    output += state[key_idx] * Float32(mQ[key_token * K + key_idx]) * scale
...
def gated_delta_rule_fwd_cute(q, k, v, g, beta, initial_state, output_final_state, scale):
    """Run the bounded dense scalar-gated delta-rule recurrence."""
    B, T, H, _ = k.shape
    HV = v.shape[2]
    out = v.new_empty(v.shape)
    final_state = q.new_empty((B, HV, 32, 32), dtype=torch.float32) if output_final_state else None
    compiled = _compile_gated_delta_rule_fwd(
...
__all__ = ["gated_delta_rule_fwd_cute"]
