...
def _can_use_cute_chunk_h_parallel(
    k: torch.Tensor,
    v: torch.Tensor,
    g: torch.Tensor | None,
    ...
    B, T, H, K = k.shape
    V = v.shape[-1]
    if B < 1 or T < 1 or H < 1 or K < 1 or V < 1 or chunk_size < T or K > 64 or V > 64:
        return False
...
def chunk_fwd_h(
    ...
    if _can_use_cute_chunk_h_parallel(k, v, g, gk, gv, h0, cu_seqlens, chunk_size, output_final_state):
        from fla.ops.backends.cute.chunk_h_parallel import chunk_h_parallel_fwd_cute

        h, ht = chunk_h_parallel_fwd_cute(k=k, v=v, h0=h0, output_final_state=output_final_state)
    ...
    def grid(meta):
        return (triton.cdiv(K, meta["BK"]) * triton.cdiv(V, meta["BV"]), NT, B * H)
...
def chunk_bwd_dh(
    ...
    def grid(meta):
        return (triton.cdiv(K, meta["BK"]) * triton.cdiv(V, meta["BV"]), NT, B * HQ)
