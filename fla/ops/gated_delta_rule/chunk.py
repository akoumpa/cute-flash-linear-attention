...
def _can_use_cute_gated_delta_rule(
    q,
    k,
    v,
    g,
    ...
    if (
        ...
        or HV % H != 0
        or B * HV < 4
        or (T > 1 and (initial_state is not None or output_final_state))
        or (q.dtype == torch.bfloat16 and (initial_state is not None or output_final_state))
        or (q.dtype == torch.bfloat16 and T > 1)
        or K != 32
        or V != 32
        or v.shape != (B, T, HV, V)
        or g.shape != (B, T, HV)
    ...
    if initial_state is not None and (
        ...
        return False
    if output_final_state and B * HV * K * V > 2**31 - 1:
        return False
...
def chunk_gated_delta_rule(
    ...
    **kwargs,
    ...
    if q.shape[2] != k.shape[2]:
        raise ValueError(
            f"q and k must have the same number of heads, but got q.shape[2]={q.shape[2]} and k.shape[2]={k.shape[2]}"
        )
    H, HV = q.shape[2], v.shape[2]
    if HV % H != 0:
        raise ValueError(
            f"For GVA, num_v_heads (HV={HV}) must be evenly divisible by num_heads (H={H}), but got HV % H = {HV % H}"
        )
    ...
    if scale is None:
        scale = k.shape[-1] ** -0.5
    if _can_use_cute_gated_delta_rule(
        q=q,
        k=k,
        v=v,
        g=g,
