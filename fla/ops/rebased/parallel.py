...
def _can_use_cute_rebased_forward(q, k, v, scale):
    if (
        not q.is_cuda
        ...
        or not all(t.device == q.device and t.is_contiguous() for t in (q, k, v))
    ):
        return False
...
class ParallelBasedFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, scale):
        if _can_use_cute_rebased_forward(q, k, v, scale):
            from fla.ops.backends.cute.rebased import rebased_forward_cute

            o, z = rebased_forward_cute(q, k, v, scale)
