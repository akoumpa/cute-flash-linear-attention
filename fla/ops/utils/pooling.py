...
def mean_pooling_fwd(
    ...
    if (
        ...
        and 0 < chunk_size <= 128
        and D <= 512
    ):
        from fla.ops.backends.cute import is_cute_dsl_available

        if is_cute_dsl_available():
            from fla.ops.backends.cute.pooling import dense_mean_pooling_fwd_cute

            return dense_mean_pooling_fwd_cute(x, chunk_size)
...
def mean_pooling_bwd(
    ...
    if (
        ...
        and do.dtype in (torch.float16, torch.bfloat16, torch.float32)
        and 0 < chunk_size <= 128
        and D <= 1024
    ):
        from fla.ops.backends.cute import is_cute_dsl_available
        ...
        if is_cute_dsl_available():
            from fla.ops.backends.cute.pooling import dense_mean_pooling_bwd_cute

            return dense_mean_pooling_bwd_cute(do, batch_size, seq_len, chunk_size)
    if chunk_indices is None and cu_seqlens is not None:
        chunk_indices = prepare_chunk_indices(cu_seqlens, chunk_size)
    NT = triton.cdiv(T, BT) if cu_seqlens is None else len(chunk_indices)
