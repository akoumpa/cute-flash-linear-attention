...
def _can_use_cute_solve_tril(
    ...
    if (
        ...
        # The single-kernel 32x32 solve crosses the autotuned Triton pipeline
        # only once enough independent blocks amortize its heavier CTA setup.
        or A.shape[0] * A.shape[2] * (A.shape[1] // 32) < 2048
    ):
        return False
...
def solve_tril(
    ...
    if _can_use_cute_solve_tril(A, cu_seqlens, chunk_indices, output_dtype):
        from fla.ops.backends.cute.solve_tril import solve_tril_32_cute

        return solve_tril_32_cute(A, output_dtype)
